...
class PacketFilter(base.Nca47Object):
    ...
    def create(self, context, values):
        pf = self.db_api.create(PacketFilterModel, values)
        return pf

    def update(self, context, id, values):
        pf = self.db_api.update_object(PacketFilterModel, id, values)
        return pf

    def get_object(self, context, **values):
        pf = self.db_api.get_object(PacketFilterModel, **values)
        return pf

    def delete(self, context, id):
        pf = self.db_api.delete_object(PacketFilterModel, id)
        return pf

    def get_objects(self, context, **values):
        pf = self.db_api.get_objects(PacketFilterModel, **values)
        return pf
